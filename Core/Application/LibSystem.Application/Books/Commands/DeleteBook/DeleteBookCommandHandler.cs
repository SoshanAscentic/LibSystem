    ...
    public class DeleteBookCommandHandler : IRequestHandler<DeleteBookCommand, Result>
    {
        ...
        public async Task<Result> Handle(DeleteBookCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var isCurrentlyBorrowed = await _borrowingRepository.IsBookCurrentlyBorrowedAsync(bookId, cancellationToken);
                ...
                _logger.LogInformation("Successfully deleted book: {Title} (ID: {BookId})", book.Title, book.BookId.Value);

                return Result.Success();
            }
            catch (BookNotFoundException ex)
            {
                _logger.LogWarning(ex, "Book not found for deletion: {BookId}", request.BookId);
                return Result.Failure(ex.Message);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error deleting book with ID: {BookId}", request.BookId);
